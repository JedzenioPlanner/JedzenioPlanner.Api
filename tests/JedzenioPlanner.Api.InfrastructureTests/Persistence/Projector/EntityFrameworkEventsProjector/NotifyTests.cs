...
namespace JedzenioPlanner.Api.InfrastructureTests.Persistence.Projector.EntityFrameworkEventsProjector
{
    public class NotifyTests
    {
        ...
        [Fact]
        public async Task Does_Notify_About_New_Object_Work()
        {
            ...
            {
                await projector.Notify(Guid.Parse("fa710953-2128-47c6-bef5-391ce66d0404"), i);
            }

            target.Verify(x=>x.Add(It.Is<Person>(y=>y.IsDeepEqual(expectedV0))), Times.Once);
            target.Verify(x=>x.Add(It.Is<Person>(y=>y.IsDeepEqual(expectedV1))), Times.Once);
            target.Verify(x=>x.Add(It.Is<Person>(y=>y.IsDeepEqual(expectedV2))), Times.Once);
            target.Verify(x=>x.Add(It.IsAny<Person>()), Times.Exactly(3));
            target.Verify(x=>x.SaveChanges(), Times.Exactly(3));
        }

        [Fact]
        public async Task Does_Notify_To_New_Version_Work()
        {
            var target = new Mock<IEventProjectorTarget<Person>>();
            var projector = new EntityFrameworkEventsProjector<Person>(_context, new []{target.Object});
            ...
            await projector.Notify(Guid.Parse("fa710953-2128-47c6-bef5-391ce66d0404"), 4);
        ...
        [Fact]
        public async Task Does_Notify_Not_To_Add_Removed_Entity()
        {
           var target = new Mock<IEventProjectorTarget<Person>>();
            var projector = new EntityFrameworkEventsProjector<Person>(_context, new []{target.Object});
            await _context.Events.AddRangeAsync(new EventDatabaseEntity
            ...
            await projector.Notify(Guid.Parse("fa710953-2128-47c6-bef5-391ce66d0404"), 3);
        ...
        [Fact]
        public async Task Does_Notify_To_Remove_Removed_Entity()
        {
            var target = new Mock<IEventProjectorTarget<Person>>();
            var projector = new EntityFrameworkEventsProjector<Person>(_context, new []{target.Object});
            var actual = new Person
            ...
            await projector.Notify(Guid.Parse("fa710953-2128-47c6-bef5-391ce66d0404"), 5);
